...
def count_clients():
    cmd = 'hostapd_cli -i enb%s-wlan1 list_sta | wc -l' % NODE_ID
...
def rssi_scenario(mac, rssi):
    ...
    if 'ASSOC' in msg:
        ...
        if bssid != bssid_[0]:
            ...
            print("Sending the following msg: %s" % msg1)
            publish_mosq_car(mac, bssid, rssi, retain='-r')
    publish_mosq_bs(bssid, count_clients(), retain='-r')
...
def pkt_callback(pkt):
    if hasattr(pkt, 'dBm_AntSignal') and pkt.dBm_AntSignal:
        mac = pkt.addr2
