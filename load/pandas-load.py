...
latency_loadd = 'grep -r \"ms\" %s | awk \'{print $7}\' | tr -d \"time=\"' % file1
icmp_seq_loadd = 'grep -r \"ms\" %s | awk \'{print $5}\' | tr -d "icmp_seq="' % file1
...
address = subprocess.Popen(latency_loadd, stdout=subprocess.PIPE, shell=True)
(out, err) = address.communicate()
latency_loadd_ = out.decode().split('\n')

address = subprocess.Popen(icmp_seq_loadd, stdout=subprocess.PIPE, shell=True)
(out, err) = address.communicate()
icmp_seq_loadd_ = out.decode().split('\n')
...
for n in range(0, 251):
    if str(n) in icmp_seq_loadd_:
        icmp_seq_loadd.append(float(icmp_seq_loadd_[id]))
        latency_loadd.append(float(latency_loadd_[id]))
        id += 1
    else:
        icmp_seq_loadd.append(float(n))
        latency_loadd.append(None)
...
x = np.array((latency_loadd + latency), dtype=np.float64)
...
fig_name = "latency-load.eps"

Data = {'latency_loadd': latency_loadd,
        'icmp_seq_loadd': icmp_seq_loadd,
        'icmp_seq': icmp_seq,
        'latency': latency
       }

df = DataFrame(Data, columns=['latency_loadd', 'icmp_seq_loadd', 'latency', 'icmp_seq'])
color = ['dodgerblue', 'orangered']

# gca stands for 'get current axis'
ax = plt.gca()

df.plot(kind='line', ls='--', x='icmp_seq_loadd', y='latency_loadd', color=color[0], ax=ax, label='Distributed Control')
...
enb1_d_pos = [0, 44]
enb2_d_pos = [enb1_d_pos[1], 164]
enb3_d_pos = [enb2_d_pos[1], 250]

enb1_c_pos = [0, 142]
enb2_c_pos = [enb1_c_pos[1], 180]
enb3_c_pos = [enb2_c_pos[1], 250]
