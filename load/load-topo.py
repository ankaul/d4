...
def topology():
    ...
    enb_pos = ['2400,3600,0', '2600,3150,0', '2900,3000,0']
    ...
    enb1 = net.addAccessPoint('enb1', mac='00:00:00:00:00:01', ssid="handover",
                              mode="a", channel="36", passwd='123456789a', encrypt='wpa2',
                              dpid='1', position=enb_pos[0],
                              inNamespace=True, country_code='DE',
                              netcfg=True, json=json_file, thriftport=50001,
                              switch_config=config1, cls=P4AP)
    enb2 = net.addAccessPoint('enb2', mac='00:00:00:00:00:02', ssid="handover",
                              mode="a", channel="36", passwd='123456789a', encrypt='wpa2',
                              dpid='2', position=enb_pos[1],
                              inNamespace=True, country_code='DE',
                              color='r', netcfg=True, json=json_file, thriftport=50002,
                              switch_config=config2, cls=P4AP)
    enb3 = net.addAccessPoint('enb3', mac='00:00:00:00:00:03', ssid="handover",
                              mode="a", channel="36", passwd='123456789a', encrypt='wpa2',
                              dpid='3', position=enb_pos[2],
                              inNamespace=True, country_code='DE',
                              netcfg=True, json=json_file, thriftport=50003,
    ...
    wan0 = net.addHost('wan0', ip='192.168.254.100/24')
    ...
    net.setPropagationModel(model="logDistance", exp=2.4)
    ...
    enb1.cmd("sysctl -w net.ipv6.conf.all.disable_ipv6=1")
    enb1.cmd("sysctl -w net.ipv6.conf.default.disable_ipv6=1")
    enb1.cmd("sysctl -w net.ipv6.conf.lo.disable_ipv6=1")
    enb2.cmd("sysctl -w net.ipv6.conf.all.disable_ipv6=1")
    enb2.cmd("sysctl -w net.ipv6.conf.default.disable_ipv6=1")
    enb2.cmd("sysctl -w net.ipv6.conf.lo.disable_ipv6=1")
    enb3.cmd("sysctl -w net.ipv6.conf.all.disable_ipv6=1")
    enb3.cmd("sysctl -w net.ipv6.conf.default.disable_ipv6=1")
    enb3.cmd("sysctl -w net.ipv6.conf.lo.disable_ipv6=1")

    for car in net.cars:
        car.cmd("sysctl -w net.ipv6.conf.all.disable_ipv6=1")
        car.cmd("sysctl -w net.ipv6.conf.default.disable_ipv6=1")
        car.cmd("sysctl -w net.ipv6.conf.lo.disable_ipv6=1")
    ...
    for id in range(1, 2):
        makeTerm(net.cars[id-1], title='car-sub', cmd="bash -c 'python car-sub.py %d %s;'" % (id, SCENARIO))
