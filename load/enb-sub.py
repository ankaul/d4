...
def mosquitto_publish(topic, bssid):
    cmd = '{} {} mosquitto_pub -h {} -t {} -m {} -r'.format(MININET_WIFI_DIR, NODE_NAME, MQTT_SERVER, topic, bssid)
    os.system(cmd)


def deauth(mac):
    print("Deauthenticating {}".format(mac))
    cmd = "{} {} hostapd_cli -i enb{}-wlan1 deauthenticate {}".format(MININET_WIFI_DIR, NODE_NAME, NODE_ID, mac)
    os.system(cmd)
