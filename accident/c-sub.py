...
EMERGENCY_TOPICS = ['02:00:00:00:00:00-emergency',
                    '02:00:00:00:02:00-emergency',
                    '02:00:00:00:04:00-emergency',
                    '02:00:00:00:06:00-emergency',
                    '02:00:00:00:08:00-emergency',
                    '02:00:00:00:0a:00-emergency',
                    '02:00:00:00:0c:00-emergency',
                    '02:00:00:00:0e:00-emergency',
                    '02:00:00:00:10:00-emergency',
                    '02:00:00:00:12:00-emergency']
...
def pub_v2v(topic, t_neigh, infra):
    ...
    if int(infra[1]) < (int(t_infra[1]) + 1) and int(t_infra[1]) >= -80:
        msg = '{}'.format(t_neigh)
        now = datetime.now()
...
def process_v2v_scenario(msg):
    if msg.topic in MAC_LIST_ADHOC_TARGET:
        ...
        t_infra = get_msg(t_infra_mac).split(',')
        if BSSID_LIST[NODE_ID-1] == infra[0] and int(infra[1]) < -82 and int(t_infra[1]) >= -85:
            if neigh == '0':
...
def process_emergency_scenario(msg):
    global MSG_SENT
    mac = _macMatchRegex.findall(msg.topic)[0]
    bssid = get_msg(mac).split(',')[0]
    if int(bssid[-1]) == NODE_ID and msg.payload.decode('utf-8') != '0':
        mac_veh = _macMatchRegex.findall(msg.topic)[0]
        route = 'route2'
        for veh in EMERGENCY_TOPICS:
            if mac_veh != veh:
                if int(float(DELAYED)) != 1000:
                    sleep(float(DELAYED))
                topic = '{}-set-route'.format(mac_veh)
                logging.info("Publishing {} to {}...".format(route, topic))
                mosquitto_publish(topic, route)
                topic = '{}-emergency'.format(mac_veh)
                logging.info("Publishing {} to {}...".format('0', topic))
                mosquitto_publish(topic, '0')
                msgTime = 'Controller %s: message sent by the Controller when T=%s' % (NODE_ID, datetime.utcnow().strftime(
                    '%Y-%m-%d %H:%M:%S.%f')[:-3])
                if not MSG_SENT:
                    if int(float(DELAYED)) != 1000:
...
def process_rssi_scenario(msg):
    ...
    if bssid != t_bssid:
        ...
        elif int(bssid) == 0 and t_bssid == BSSID_LIST[NODE_ID - 1]:
            msg1 = '{}'.format(infra_mac)
            topic = msg1 + '-handover'
...
def on_connect(client, userdata, flags, rc):
    ...
    if SCENARIO == 'distributed':
        topic_list = EMERGENCY_TOPICS + MAC_LIST_TARGET + MAC_LIST_ADHOC_TARGET
...
def on_message(client, userdata, msg):
    ...
    if SCENARIO == 'distributed':
        if 'emergency' in msg.topic:
            process_emergency_scenario(msg)
        else:
