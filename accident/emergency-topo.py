...
def topology():
    ...
    arg = sys.argv
    SCENARIO = arg[1]
    DELAYED = arg[2] if len(arg) > 2 else 1000
    host_ip = '172.210.0.1'
    ...
    enb1 = net.addAccessPoint('enb1', mac='00:00:00:00:00:01', ssid="handover",
                              ...
                              bssid_list=[['00:00:00:00:00:02', '00:00:00:00:00:03']],
    ...
    enb2 = net.addAccessPoint('enb2', mac='00:00:00:00:00:02', ssid="handover",
                              mode="a", channel="40", datapath='user',
                              passwd='123456789a', encrypt='wpa2',
                              bssid_list=[['00:00:00:00:00:01', '00:00:00:00:00:03']],
    ...
    enb3 = net.addAccessPoint('enb3', mac='00:00:00:00:00:03', ssid="handover",
                              mode="a", channel="36", datapath='user',
                              passwd='123456789a', encrypt='wpa2',
                              bssid_list=[['00:00:00:00:00:01', '00:00:00:00:00:02']],
    ...
    if 'distributed' in SCENARIO:
        makeTerm(h1, title='c-sub-pub', cmd="bash -c 'python c-sub.py 1 %s %s;'" % (SCENARIO, DELAYED))
        makeTerm(h2, title='c-sub-pub', cmd="bash -c 'python c-sub.py 2 %s %s;'" % (SCENARIO, DELAYED))
        makeTerm(h3, title='c-sub-pub', cmd="bash -c 'python c-sub.py 3 %s %s;'" % (SCENARIO, DELAYED))
    ...
    for id in range(1, 11):
        ...
        makeTerm(net.cars[id-1], title='car-sub', cmd="bash -c 'python car-sub.py %d %s %s %s %s;'" % (id, SCENARIO, SOCKET, SERVER, DELAYED))
    makeTerm(net.cars[0], title='car-sender', cmd="bash -c 'python car-sender-v2x.py %d %s;'" % (1, SCENARIO))

    for id in range(0, 1):
        makeTerm(net.cars[id], title='sumoClient', cmd="bash -c 'python secondClient-traffic-jam.py %s %s %s;'" % (1, SCENARIO, DELAYED))
    ...
    if int(float(DELAYED)) != 1000:
        file = 'timestamp-%s-delayed.txt' % SCENARIO
    else:
        file = 'rm timestamp-%s.txt' % SCENARIO
    if glob.glob(file):
